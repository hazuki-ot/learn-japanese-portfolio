...
@section('content')
<div class="row">
    @forelse ($category->hiragana as $word)
        <div class="col-auto me-3 mb-4">
            {{-- +each language sound button --}}
            ...
            @php
                ...
                $sound = optional(
                    $word->hiraganaSound->firstWhere('sound.language_id', session('user_language'))
                )->sound;
            @endphp
    ...
    @empty
        <div class="row">
            <p class="h3">There is no ひらがな word.</p>
        </div>
    @endforelse
</div>
